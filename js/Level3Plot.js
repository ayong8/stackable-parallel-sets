...
function Level3Plot() {
  ...
  function _level3Plot(selection) {
    ...
    const coorArcs = selection
      .append('g')
      .attr('class', 'g_arcs')
      .attr('transform', 'translate(' + xWordScale.bandwidth() / 2 + ',' + (lCom.hPlot.wordPlot.h/2) + ')')
      .selectAll('.word_arc')
      ...
      .attr('d', d => {
        ...
        return draw(
          ...
          0,
          xWordScale(wordOnLeft) + (xWordScale(wordOnRight) - xWordScale(wordOnLeft)) / 2,
          50 * (1 + (xWordScale(wordOnRight) - xWordScale(wordOnLeft)) / lCom.hPlot.w),
    ...
    const xWordAxisSetting = d3.axisBottom(xWordScale),
      xWordAxis = d3
        ...
        .attr('transform', 'translate(0,' + (lCom.hPlot.wordPlot.h/2) + ')')
        .selectAll('text')
    ...
    selection
      ...
      .attr('y', d => lCom.hPlot.wordPlot.h/2 - wordCountScale(d.countTotal))
